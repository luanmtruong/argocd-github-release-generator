...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Ci":
		...
		case "BaseContainer":
			...
			return (*Ci).BaseContainer(&parent, ctx, src), nil
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Ci", dagger.TypeDefWithObjectOpts{SourceMap: dag.SourceMap("main.go", 7, 6)}).
					WithFunction(
						dag.Function("BaseContainer",
							dag.TypeDef().WithKind(dagger.TypeDefKindVoidKind)).
							WithSourceMap(dag.SourceMap("main.go", 9, 1)).
							WithArg("src", dag.TypeDef().WithKind(dagger.TypeDefKindVoidKind), dagger.FunctionWithArgOpts{SourceMap: dag.SourceMap("main.go", 9, 49)}))), nil
	default:
